    ...
    <el-row type="flex">
      <el-col :span="15">
        ...
        <enterprise-information-table 
            :enterpriseTableData="builderTableData"
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //点击确定进行修改保存
      submitUpdateForm() {
        if(!this.enterpriseForUpdateForm.username||!this.enterpriseForUpdate.name){
          this.$refs.msgDialog.confirm("用户名和企业名称均不能为空")
        }else{
      ...
      //删除企业信息
      delRecord() {
        var url=this.HOST + "/deleteBuilders?ids=" + this.ids
      ...
      findAllBuilders() {
        ...
        this.$http.get(url).then(response => {
          this.builderTableData = response.data.rows;
